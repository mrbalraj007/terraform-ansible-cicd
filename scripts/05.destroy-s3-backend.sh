...
# 3. Empties the bucket (all versions, delete markers, fragments)
...
while true; do
  ...
  echo "$VERSION_JSON" | jq -r '(.Versions // [])[] | "\(.Key)|\(.VersionId)"' 2>/dev/null | \
    while IFS='|' read -r KEY VID; do
      aws s3api delete-object --bucket "$BUCKET_NAME" --key "$KEY" --version-id "$VID" >/dev/null 2>&1
      echo "  Deleted version: $KEY (v:$VID)"
    done

  echo "$VERSION_JSON" | jq -r '(.DeleteMarkers // [])[] | "\(.Key)|\(.VersionId)"' 2>/dev/null | \
    while IFS='|' read -r KEY VID; do
      aws s3api delete-object --bucket "$BUCKET_NAME" --key "$KEY" --version-id "$VID" >/dev/null 2>&1
      echo "  Deleted delete-marker: $KEY (v:$VID)"
    done
done
