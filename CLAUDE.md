...
- **CloudWatch monitoring** (root `main.tf`): IAM role + instance profile with `CloudWatchAgentServerPolicy`, SNS topic + email subscription, metric alarms (CPU/Memory/Disk at 80% threshold, 2 evaluation periods). Alarms use `for_each` (not `count`) — each server instance gets individually addressable resources. Alarm naming: `EC2 <name>-<env>-<N>-{CPU,Memory,Disk}-Alerts`.
